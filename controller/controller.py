#API WEB FASTAPI
from fastapi import APIRouter, Request, Response  # ← SOLO CAMBIO 1: FastAPI → APIRouter
...
app = APIRouter()  # ← SOLO CAMBIO 2: FastAPI() → APIRouter()
...
def dashboard(request: Request):
    ...
    if not request_session_id:
        myviewcomponent.get_index_view(request)  # ← FALTA return aquí
    else:
...
async def add_song(song_data: dict):
    try:
        result = mymodelcomponent.add_song(song_data)
...
@app.post("/genres")
async def add_genre(genre_data: dict):
    try:
        result = mymodelcomponent.add_genre(genre_data)
        return {"success": True, "data": result}
    except Exception as e:
        return {"success": False, "error": str(e)}
